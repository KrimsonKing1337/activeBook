...
export class ModalContent {
  constructor() {
    ...
    this.$sectionContent = this.$modalContent.find('.js-section-content');
    this.$sectionConfirmButtons = this.$modalContent.find('.js-section-confirm-buttons');
    this.$sectionConfirmButtonAccept = this.$modalContent.find('.js-confirm-accept');
    this.$sectionConfirmButtonDecline = this.$modalContent.find('.js-confirm-decline');
  ...
  init(effect) {
    ...
    this.setConfirmButtonsHandlers();
    this.set(effect);
  ...
  async reset() {
    ...
    this.$sectionContent.removeClass('active');
    this.$sectionContent.html('');
   ...
   * @param [withConfirm] {boolean};
   */
  set({src, poster, modalContentType, withConfirm = false} = {}) {
    if (modalContentType === 'image') {
      ...
      ModalContent.setHtml(this.$sectionContent, src);
      ModalContent.showElem(this.$sectionWrapper);

      if (withConfirm === true) {
        ModalContent.showElem(this.$sectionConfirmButtons);
      }
  ...
  /**
   *
   * @param [handlerForAcceptBtn] {function}
   * @param [handlerForDeclineBtn] {function}
   */
  setConfirmButtonsHandlers(handlerForAcceptBtn = () => {}, handlerForDeclineBtn = () => {}) {
    this.$sectionConfirmButtonAccept.on('click', () => {
      handlerForAcceptBtn();

      this.close();
    });

    this.$sectionConfirmButtonDecline.on('click', () => {
      handlerForDeclineBtn();
