...
//          - WebAudio: can't play a newFileName until it's fully loaded
// SOLUTION: Use both!
...
// For memory policy: heuristic for uncompressed audio newFileName size per minute
...
var Gapless5RequestManager = function(parentPlayer) {
	...
	this.loadingTrack = -1;		// What newFileName to consume
	var parent = parentPlayer;
	var that = this;
...
// the "newFileName" members out to constitute the that.mgr.sources[] in the Gapless5 player
var Gapless5FileList = function(inPlayList, inStartingTrack, inShuffle) {
	...
	this.add = function(index, file) {
		...
		// Shift currentItem if the insert newFileName is earlier in the list
...
//   tracks: path of newFileName (or array of music newFileName paths)
...
this.gotoTrack = function (newIndex, bForcePlay) {
	...
	else
	{
		...
		if (that.mgr.sources[oldIndex].getState() == Gapless5State.Loading)
		{
			...
			// TODO: better way to have just the newFileName list?
